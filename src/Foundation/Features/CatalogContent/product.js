export default class Product {
  ...
  deleteWishlistClick() {
    ...
    addToCartClickCT() {
        let inst = this;
        Array.from(document.querySelectorAll(".addToCartCT")).forEach(function (el, i) {
            el.setAttribute("href", "javascript:void(0);")
            el.addEventListener("click", function () {
                let code = el.getAttribute('data');
                let data = {
                ...
                inst.addToCartCT(data, '/CTDefaultCart/AddToCart', callback, true);
            });
        });
    }

    addToCartCT(data, url, callback, isAddToCart) {
        data.requestFrom = "axios";
        axios.post(url, data)
            .then(function (result) {
                if (result.data.statusCode == 0) {
